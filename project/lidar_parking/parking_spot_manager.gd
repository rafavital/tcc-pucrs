...
var total_cars: int
var parking_spots: Array[ParkSpot]
var free_parking_spots: Array[ParkSpot]
...
func get_random_free_parking_spot_transform():
	var id = randi_range(0, len(free_parking_spots) -1 )
	var spot = free_parking_spots[id]
	free_parking_spots.remove_at(id)
	return spot
	
func release_park_spot (spot):
	free_parking_spots.append(spot)
